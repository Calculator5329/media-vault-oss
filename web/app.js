...
const VIEW_LABELS={photos:'Photos',people:'People',places:'Places',map:'Map',trips:'Trips',buckets:'Buckets',albums:'Albums',quality:'Archive quality',stacks:'Similar',gaps:'Fill the gaps'};
...
async function load(append=false,start=0){
 ...
 if(state.view==='stacks'){await renderStacks(request);return}
...
async function renderPeople(ticket){
 ...
 $('content').innerHTML=`<div class="people-tools"><label class="tool"><span>Sort</span><select id="peopleSort" data-testid="people.toolbar.sort"><option value="count">Most photos</option><option value="faces">Most confirmed faces</option><option value="name">Name</option></select></label>${hiddenCount?`<button class="tool quiet" id="toggleHiddenPeople" data-testid="people.toolbar.toggle-hidden">${showHiddenPeople?'Hide hidden people':`Show ${number(hiddenCount)} hidden`}</button>`:''}</div>`+'<div class="people">'+sortPeople(peopleCache.filter(p=>showHiddenPeople||!p.hidden)).map(p=>`<button class="person-card${p.hidden?' hidden-person':''}" data-person="${p.id}" data-testid="people.card.open.${p.id}">${p.face?`<img src="/face-preview/${p.face}" alt="">`:p.cover?`<img src="/preview/${p.cover}" alt="">`:`<div class="person-placeholder">${escape(p.name.slice(0,1))}</div>`}<strong>${escape(p.name)}</strong><small>${number(p.count)} ${p.count===1?'photo':'photos'}${p.confirmed_faces?' · '+number(p.confirmed_faces)+' faces':''}${p.hidden?' · hidden':''}</small></button>`).join('')+'<button class="person-card person-add" id="createPerson" data-testid="people.card.create"><div class="person-placeholder">+</div><strong>Add a person</strong><small>Or name a group below</small></button></div>';
 ...
 $('subtitle').textContent=`${peopleCache.length} named · ${number(data.untagged)} photos without people tags. Your choices stay saved.`;
...
async function renderFaceGroups(ticket){
 ...
 section.innerHTML=`<div class="group-head review-head"><div><h2>${showIgnoredFaces?'Ignored detections':'Unnamed faces'}</h2><span>${data.ready?`${number(data.faces)} faces found in ${number(data.processed_photos)} of ${number(data.candidate_photos)} photos · ${number(data.confirmed)} already named`:'Waiting for the first face scan'}</span></div><span class="spacer"></span><label class="tool"><span>Show</span><select id="faceShow" data-testid="faces.toolbar.show"><option value="groups">Groups (${number(data.groups.length-data.single_groups)})</option><option value="all">Groups and single faces (${number(data.single_groups)})</option><option value="ignored">Ignored</option></select></label><label class="tool"><span>Grouping</span><select id="faceSensitivity" data-testid="faces.toolbar.sensitivity"><option value="tight">Tighter</option><option value="balanced">Balanced</option><option value="loose">Looser</option></select></label><button class="tool quiet" id="browseUntagged" data-testid="faces.toolbar.browse-untagged">Tag whole photos</button></div>`
 ...
 $('browseUntagged').onclick=()=>{openCollection('untagged','','Photos without people tags');state.selecting=true;selectionTools()};
...
function renderDetailStack(item){
 ...
 box.innerHTML=`<p class="local-note">${stack.confirmed?'In a stack you confirmed':'Proposed stack'} of ${stack.count} similar shots${stack.top?' · this is the top':''}.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="chip" id="openStack" data-testid="inspector.actions.open-stack">Show all ${stack.count}</button><button class="outline" id="reviewStacks" data-testid="stacks.toolbar.review">Review stacks</button></div>`;
 ...
 $('reviewStacks').onclick=()=>{$('inspector').close();state.view='stacks';load()};
}

async function renderStacks(ticket){
 $('title').textContent='Similar shots';$('subtitle').textContent='Near-duplicates and bursts, stacked so the grid shows one of each.';
 try{const data=await api('/api/stacks');if(ticket!==requestId||state.view!=='stacks')return;
 const tile=(s,i)=>`<button class="tile stack-member" data-stack-member="${s.id}" data-content="${i.content_hash}" data-testid="stacks.member.open.${i.content_hash}" title="${i.top?'Top of the stack':'Make this the top'}" aria-label="${escape(i.name)}${i.top?' (top)':''}"><img loading="lazy" src="/preview/${i.id}" alt="${escape(i.name)}">${i.top?'<span class="duration">Top</span>':''}<span class="tile-title">${escape(i.name)}${i.width?` · ${i.width}×${i.height}`:''}</span></button>`;
 const row=s=>`<article class="stack-row" data-stack="${s.id}"><div class="stack-head"><strong>${s.day?escape(dateLabel(s.day)):'Undated'} · ${s.count} shots</strong><span class="muted">${s.confirmed?'Confirmed by you':s.collapse?'Proposed from image similarity':'Proposed, undated: shown separately until you decide'}</span><span style="flex:1"></span>${s.confirmed?`<button class="outline" data-unstack="${s.id}" data-testid="stacks.card.unstack.${s.id}">Unstack</button>`:`<button class="chip" data-confirm="${s.id}" data-testid="stacks.card.confirm.${s.id}">Keep stacked</button><button class="outline" data-unstack="${s.id}" data-testid="stacks.card.separate.${s.id}">Keep separate</button>`}<button class="outline" data-open-stack="${s.id}" data-testid="stacks.card.open.${s.id}">Browse</button></div><div class="grid stack-grid">${s.items.map(i=>tile(s,i)).join('')}</div></article>`;
 const proposed=data.stacks.filter(s=>!s.confirmed),confirmed=data.stacks.filter(s=>s.confirmed);
 $('content').innerHTML=`<p class="local-note">${escape(data.meaning)} ${data.ready?`${number(data.hashed)} photos hashed so far.`:'Image hashing has not started yet; stacks appear as the enrichment worker hashes photos.'}</p>`
  +(proposed.length?`<div class="group-head"><h2>Proposed stacks</h2><span>${number(proposed.length)}</span></div>`+proposed.slice(0,stackLimit).map(row).join('')+(proposed.length>stackLimit?'<button class="outline" id="moreStacks" data-testid="stacks.list.more" style="margin:20px 0">More stacks</button>':''):'<div class="empty"><h2>No proposed stacks</h2><p>Nothing similar is waiting for a decision.</p></div>')
  +(confirmed.length?`<div class="group-head"><h2>Confirmed stacks</h2><span>${number(confirmed.length)}</span></div>`+confirmed.map(row).join(''):'');
 const byId=Object.fromEntries(data.stacks.map(s=>[s.id,s]));
 const act=async(b,op,payload,message)=>{b.disabled=true;try{await saveOrganization(op,payload);notify(message);await renderStacks(requestId)}catch(e){notify(e.message);b.disabled=false}};
 $('content').querySelectorAll('[data-unstack]').forEach(b=>b.onclick=()=>act(b,'unstack',{stack:b.dataset.unstack},byId[b.dataset.unstack].confirmed?'Stack removed':'Kept separate'));
 $('content').querySelectorAll('[data-confirm]').forEach(b=>b.onclick=()=>{const s=byId[b.dataset.confirm];act(b,'stack',{stack:s.id,contents:s.contents,top:s.top},'Stack confirmed')});
 $('content').querySelectorAll('[data-stack-member]').forEach(b=>b.onclick=()=>{const s=byId[b.dataset.stackMember];if(b.dataset.content===s.top)return;act(b,'stack',{stack:s.id,contents:s.contents,top:b.dataset.content},'Top chosen; stack confirmed')});
 $('content').querySelectorAll('[data-open-stack]').forEach(b=>b.onclick=()=>{state.view='photos';state.stack=b.dataset.openStack;state.collectionName='';state.filter='all';load()});
 if($('moreStacks'))$('moreStacks').onclick=()=>{stackLimit+=40;renderStacks(requestId)};
 $('content').querySelectorAll('img').forEach(img=>img.addEventListener('error',()=>img.closest('.tile').classList.add('failed'),{once:true}));
 }catch{$('content').innerHTML='<div class="empty">Similar shots could not be loaded. Try again.</div>'}
}
let stackLimit=40;
...
async function renderEverything(request){
 ...
 try{
  ...
  html+=section('People',data.people.total,'<div class="people search-people">'+data.people.items.map(p=>`<button class="person-card" data-person="${p.id}" data-testid="search.person.open.${p.id}">${p.face?`<img src="/face-preview/${p.face}" alt="">`:p.cover?`<img src="/preview/${p.cover}" alt="">`:`<div class="person-placeholder">${escape(p.name.slice(0,1))}</div>`}<strong>${escape(p.name)}</strong><small>${number(p.count)} ${p.count===1?'photo':'photos'}</small></button>`).join('')+'</div>');
